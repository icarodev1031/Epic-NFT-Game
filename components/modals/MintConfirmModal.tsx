import { Dialog, Transition } from "@headlessui/react";
import { Fragment } from "react";
import { useDapp } from "../../contexts/DappContext";
import { CharacterProps } from "../../utils/contracts";

export const MintConfirmationModal = ({
  isOpen,
  closeModal,
  item,
}: {
  isOpen: boolean;
  closeModal: () => void;
  item: CharacterProps;
}) => {
  const { mintCharacterNFT } = useDapp();
  return (
    <>
      <Transition appear show={isOpen} as={Fragment}>
        <Dialog
          as="div"
          className="fixed inset-0 z-10 overflow-y-auto"
          onClose={closeModal}
        >
          <div className="min-h-screen px-4 text-center">
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0"
              enterTo="opacity-100"
              leave="ease-in duration-200"
              leaveFrom="opacity-100"
              leaveTo="opacity-0"
            >
              <Dialog.Overlay className="fixed inset-0" />
            </Transition.Child>

            {/* This element is to trick the browser into centering the modal contents. */}
            <span
              className="inline-block h-screen align-middle"
              aria-hidden="true"
            >
              &#8203;
            </span>
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0 scale-95"
              enterTo="opacity-100 scale-100"
              leave="ease-in duration-200"
              leaveFrom="opacity-100 scale-100"
              leaveTo="opacity-0 scale-95"
            >
              <div className="inline-block w-full max-w-2xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl border-red-500 border-[3px]">
                <div className="flex flex-col items-center">
                  <span className="text-3xl font-medium leading-6 text-red-500 font-avengers mt-5">
                    Are you sure you want to mint
                  </span>
                  <span className="text-4xl font-medium leading-6 text-red-500 font-avengers mt-10">
                    {item.name}
                  </span>
                  <span className="font-extrabold font-xl text-red-600 mt-5">
                    ( Price : 10 EPIC Token )
                  </span>
                </div>

                <div className="mt-10 flex flex-row justify-center space-x-5">
                  <button
                    type="button"
                    className="bg-white border-red-500 border-[3px] rounded-xl w-40 pt-3 font-avengers text-2xl text-red-500 hover:bg-red-500 hover:text-white"
                    onClick={async () => {
                      await mintCharacterNFT(item.characterIndex);
                    }}
                  >
                    Yeah
                  </button>
                  <button
                    type="button"
                    className="bg-white border-red-500 border-[3px] rounded-xl w-40 pt-3 font-avengers text-2xl text-red-500 hover:bg-red-500 hover:text-white"
                    onClick={closeModal}
                  >
                    Naah
                  </button>
                </div>
              </div>
            </Transition.Child>
          </div>
        </Dialog>
      </Transition>
    </>
  );
};
